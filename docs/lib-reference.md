...
`c64asm-reference.md` §11) — so if your project keeps `lib/` alongside
...
the top, using `.ifdef`/`.error` (`c64asm-reference.md` §12), and
...
`print_msg`'s internal loop uses `@`-prefixed local labels (§10), so its
