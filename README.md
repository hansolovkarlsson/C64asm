...
| `adventure.asm` / `.prg` / `.lst` | Demo: a small text adventure — typed commands via `CHRIN`, a room/item/puzzle state machine, room exits stored in a `.struct`-based table (`room_exits+Exits.north,x` instead of a bare offset number) with an `.assert` guarding the hand-rolled multiply that indexes into it; uses the standard library (`lib/text.inc`, `lib/input.inc`) rather than reimplementing string/input handling |
...
- **Zero-page sizing of a *forward-referenced* label** can, in rare
  cases, differ between passes — see `c64asm-reference.md` §21 for when
...
- Assembly can surface several independent errors from one run (see
  `c64asm-reference.md` §20), though messages after the first can
